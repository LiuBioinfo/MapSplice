...
void regenerateSeqOthelloExistOrNotVecVecVec(vector< vector< vector<bool> > >& seqOthelloTranscriptExistOrNotBoolVecVecVec,
	...
	int rna_num, int sample_num, vector<double>& cutoff_existKmerPercMin_vec, int KmerLength)
{
	for(int tmp = 0; tmp < cutoff_existKmerPercMin_vec.size(); tmp ++)
	{
		...
		for(int tmp1 = 0; tmp1 < rna_num; tmp1++)
		{
			...
			{
				//cout << "tmp2: " << tmp2 << endl;
				double tmpExistKmerPerc = (double)(seqOthelloResults_sampleExistKmerNumVecVec[tmp1])[tmp2]/(double)(tmpRnaLength-KmerLength+1);
...
int main(int argc, char** argv)
{
	...
	regenerateSeqOthelloExistOrNotVecVecVec(seqOthelloTranscriptExistOrNotBoolVecVecVec, seqLengthVec, 
		seqOthelloResults_sampleExistKmerNumVecVec, rna_num, sample_num, cutoff_existKmerPercMin_vec, KmerLength);
