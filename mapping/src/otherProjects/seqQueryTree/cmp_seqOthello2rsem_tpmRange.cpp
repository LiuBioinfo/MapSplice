...
void regenerateRsemExistOrNotVecVec(
	vector< vector<bool> >& rsemTranscriptValidForCmpOrNotBoolVecVec,
	vector< vector<bool> >& rsemTranscriptExistOrNotBoolVecVec, 
	vector< vector<double> >& rsemAbundanceVecVec, double cutoff_high, double cutoff_low)
{
	int rnaNum = rsemAbundanceVecVec.size();
	int sampleNum = rsemAbundanceVecVec[0].size();
	for(int tmp1 = 0; tmp1 < rnaNum; tmp1++)
	{
		vector<bool> tmpRsemTranscriptValidForCmpOrNotBoolVec;
		...
		for(int tmp2 = 0; tmp2 < sampleNum; tmp2++)
		{
			...
			tmpRsemTranscriptValidForCmpOrNotBoolVec.push_back(false);
		}
		rsemTranscriptExistOrNotBoolVecVec.push_back(tmpRsemTranscriptExistOrNotBoolVec);
		rsemTranscriptValidForCmpOrNotBoolVecVec.push_back(tmpRsemTranscriptValidForCmpOrNotBoolVec);
	}
	...
	for(int tmp1 = 0; tmp1 < rnaNum; tmp1++)
	{
		for(int tmp2 = 0; tmp2 < sampleNum; tmp2++)
		{
			...
			if(tmpAbundance >= cutoff_high)
			{
				(rsemTranscriptExistOrNotBoolVecVec[tmp1])[tmp2] = true;
				(rsemTranscriptValidForCmpOrNotBoolVecVec[tmp1])[tmp2] = true;
			}
			else if(tmpAbundance < cutoff_low)
			{	
...
int main(int argc, char** argv)
{
	if(argc != 8)
	{
		...
		cout << "#5 cutoff_rsemTpm_high" << endl;
		cout << "#6 cutoff_rsemTpm_low" << endl;
		cout << "#7 cutoff_existKmerPercMin" << endl;
		exit(1);
	}
	...
	string cutoffStr_rsemTpm_high = argv[5];
	string cutoffStr_rsemTpm_low = argv[6];
	string cutoffStr_existKmerPercMin = argv[7];
	...
	double cutoff_rsemTpm_high = atof(cutoffStr_rsemTpm_high.c_str());
	double cutoff_rsemTpm_low = atof(cutoffStr_rsemTpm_low.c_str());
	vector< vector<bool> > rsemTranscriptValidForCmpOrNotBoolVecVec;
	vector< vector<bool> > rsemTranscriptExistOrNotBoolVecVec;
	cout << "start to regenerateRsemExistOrNotVecVec" << endl;
	regenerateRsemExistOrNotVecVec(rsemTranscriptValidForCmpOrNotBoolVecVec,
		rsemTranscriptExistOrNotBoolVecVec, rsemAbundanceVecVec, cutoff_rsemTpm_high, cutoff_rsemTpm_low);
	...
	for(int tmp1 = 0; tmp1 < rna_num; tmp1++)
	{
		for(int tmp2 = 0; tmp2 < sample_num; tmp2++)
		{
			if(!((rsemTranscriptValidForCmpOrNotBoolVecVec[tmp1])[tmp2]))
				continue;
