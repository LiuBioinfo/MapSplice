...
int main(int argc, char** argv)
{
	if(argc != 7)
	{
		...
		cout << "#5 cutoff_high" << endl;
		cout << "#5 cutoff_low" << endl;
		exit(1);
	}
	...
	string cutoffStr_high = argv[5];
	string cutoffStr_low = argv[6];
	...
	double cutoff_high = atof(cutoffStr_high.c_str());
	double cutoff_low = atof(cutoffStr_low.c_str());
	vector< vector<bool> > rsemTranscriptValidForCmpOrNotBoolVecVec;
	vector< vector<bool> > rsemTranscriptExistOrNotBoolVecVec;
	regenerateRsemExistOrNotVecVec(rsemTranscriptValidForCmpOrNotBoolVecVec,
		rsemTranscriptExistOrNotBoolVecVec, rsemAbundanceVecVec, cutoff_high, cutoff_low);
