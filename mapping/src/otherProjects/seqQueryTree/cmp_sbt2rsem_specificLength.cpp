...
void generateSbtResults_foundSampleIdVecVec(vector<int>& sbtResults_lengthVec,
	vector< vector<int> >& sbtResults_foundSampleIdVecVec, string& sbtResults)
{
	...
	while(!sbt_ifs.eof())
	{
		...
		string tmpQuery_seq = tmpQuery_seq_hitNum.substr(1, tabLoc);
		int tmpQuery_seq_length = tmpQuery_seq.length();
		sbtResults_lengthVec.push_back(tmpQuery_seq_length);
...
int main(int argc, char** argv)
{
	if(argc != 8)
	{
		...
		cout << "#5 cutoff" << endl;
		cout << "#6 length_min" << endl;
		cout << "#7 length_max" << endl;
		exit(1);
	}
	...
	string length_min_str = argv[6];
	string length_max_str = argv[7];
	int length_min = atoi(length_min_str.c_str());
	int length_max = atoi(length_max_str.c_str());
	...
	vector<int> rnaSeqLengthVec;
	generateSbtResults_foundSampleIdVecVec(rnaSeqLengthVec,
		sbtResults_foundSampleIdVecVec, sbtResults);
	...
	for(int tmp1 = 0; tmp1 < rna_num; tmp1++)
	{
		if((rnaSeqLengthVec[tmp1] > length_max)||(rnaSeqLengthVec[tmp1] < length_min))
			continue;
		for(int tmp2 = 0; tmp2 < sample_num; tmp2++)
		{
	...
	cmp_ofs << "rsem_exist_sbt_exist_num:\t" << rsem_exist_sbt_exist_num << endl; // TP
	cmp_ofs << "rsem_exist_sbt_nonexist_num:\t" << rsem_exist_sbt_nonexist_num << endl; // FN
	cmp_ofs << "rsem_nonexist_sbt_exist_num:\t" << rsem_nonexist_sbt_exist_num << endl;	// FP
	cmp_ofs << "rsem_nonexist_sbt_nonexist_num:\t" << rsem_nonexist_sbt_nonexist_num << endl; // TN
	cmp_ofs << endl;
