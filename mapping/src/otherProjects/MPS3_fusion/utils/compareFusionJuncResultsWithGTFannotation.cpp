...
bool extractExonChrNamePosFromFormattedGTFstr(
	int& tmpExonChrNameInt, int& tmpExonChrStartPos, int& tmpExonChrEndPos,
	string& tmpExonStrand, string& tmpGeneName,
	string& tmpFormattedExonStr, Index_Info* indexInfo)
{
	vector<string> tmpExonFieldVec;
	int startLoc = 0;
	for(int tmp = 0; tmp < 7; tmp++)
	{
		int tabLoc = tmpFormattedExonStr.find("\t", startLoc);
		//if(tabLoc == string::npos)
		//	return false;
		tmpExonFieldVec.push_back(tmpFormattedExonStr.substr(startLoc, tabLoc - startLoc));
		startLoc = tabLoc + 1;
	}
	string tmpExon_chrNameStr = tmpExonFieldVec[0];
	string tmpExon_startPosStr = tmpExonFieldVec[1];
	string tmpExon_endPosStr = tmpExonFieldVec[2];
	tmpExonStrand = tmpExonFieldVec[3];
	if((tmpExonStrand != "+")&&(tmpExonStrand != "-"))
		return false;
	string tmpExon_featureStr = tmpExonFieldVec[4];
	if(tmpExon_featureStr != "exon")
		return false;
	tmpGeneName = tmpExonFieldVec[6];
...
int main(int argc, char** argv)
{
	...
	while(!gtf_ifs.eof())
	{
		...
		bool exonInfoExtractedSuccessFully_bool = extractExonChrNamePosFromFormattedGTFstr(tmpExonChrNameInt, 
			tmpExonChrStartPos, tmpExonChrEndPos, tmpExonStrand, tmpExonGeneName, tmpGTFstr, indexInfo);		
	...
	string outputFusionJuncWithGeneInfoPath = outputFolderStr + "fusionJunc_geneInfo.txt";
	ofstream fusionJuncWithGeneInfo_ofs(outputFusionJuncWithGeneInfoPath.c_str());
	string outputFusionJuncWithGeneInfoPath_intraGene = outputFolderStr + "fusionJunc_geneInfo_intraGene.txt";
	ofstream fusionJuncWithGeneInfo_intraGene_ofs(outputFusionJuncWithGeneInfoPath_intraGene.c_str());
	string geneList_intraGene = outputFolderStr + "geneList_intraGene.txt";
	ofstream geneList_intraGene_ofs(geneList_intraGene.c_str());
	string outputFusionJuncWithGeneInfoPath_interGene = outputFolderStr + "fusionJunc_geneInfo_interGene.txt";
	ofstream fusionJuncWithGeneInfo_interGene_ofs(outputFusionJuncWithGeneInfoPath_interGene.c_str());
	string geneList_interGene = outputFolderStr + "geneList_interGene.txt";
	ofstream geneList_interGene_ofs(geneList_interGene.c_str());

	string outputFusionJuncWithGeneInfoPath_outOfGene = outputFolderStr + "fusionJunc_geneInfo_outOfGene.txt";
	...
	while(!fusionJunc_ifs.eof())
	{
		...
		string tmpStrand_2;
		//int tmpSupNum;
		extractFusionJuncChrNamePosFromFusionJuncStr(tmpChrNameInt_1, tmpChrNameInt_2, tmpChrPosInt_1, tmpChrPosInt_2, //tmpSupNum,
			tmpStrand_1, tmpStrand_2, tmpFusionJuncStr, indexInfo);
		...
		for(int tmp = 0; tmp < tmpRegionVecSize_1; tmp++)
		{
				...
				&&(tmpChrPosInt_1 - offsetInt <= tmpRegion_endPos))
			{
				...
				{
					if((tmpStrand_1 == tmpRegion_strand)||(tmpStrand_1 == "N"))			
		...
		for(int tmp = 0; tmp < tmpRegionVecSize_2; tmp++)
		{
				...
				&&(tmpChrPosInt_2 - offsetInt <= tmpRegion_endPos))
			{
				if(checkStrandOrNot_bool)
				{
					if((tmpStrand_2 == tmpRegion_strand)||(tmpStrand_2 == "N"))
