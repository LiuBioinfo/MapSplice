...
void simplifyClassifiedJuncFile(string& SJ_detailInfo_kept, string& simplifiedSJfile)
{
...
int main(int argc, char**argv)
{
	if(argc != 6)
	{
		cout << "Executable <InputIndexInfo> <threadNum> <OutputFolder> <inputSAM> <outputJunc>" << endl;
		exit(1);
	}
	...
	cout << "start to report junctions ......" << endl;
	log_ofs << endl << "[" << asctime(local) << "...";
	log_ofs << "start to report junctions ......" << endl;
	log_ofs << endl << "[" << asctime(local) << "...";
