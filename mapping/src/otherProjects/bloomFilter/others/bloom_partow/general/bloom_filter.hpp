...
public:
  ...
  void buildFromFaFile(string& inputFaFile, ofstream& log_ofs)
  {
        ifstream inputFa_ifs(inputFaFile.c_str());
        int tmpLineNO = 0;
        while(!inputFa_ifs.eof())
        {
          string tmp_name, tmp_seq;
          getline(inputFa_ifs, tmp_name);
          tmpLineNO ++;
