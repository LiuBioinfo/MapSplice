...
int main(int argc, char** argv)
{
	...
	alignInferJunctionHashInfoVec->insertJuncFromAlignmentFile_chrNamePos_supportNum_anchorSize_parallel(
	...
	alignInferJunctionHashInfoVec->mergeAlignInferJuncHashInfoInVec2one_chrNamePos_supportNum_anchorSize(
	...
	for(int tmpJunc = 0; tmpJunc < juncNum; tmpJunc ++)
	{
		...
		int tmpJunc_anchorSize_doner = alignInferJunctionHashInfo_merged->returnAnchorSizeMax_doner(tmpJunc);
		int tmpJunc_anchorSize_acceptor = alignInferJunctionHashInfo_merged->returnAnchorSizeMax_acceptor(tmpJunc);
		...
		if(tmpAlterDonerSpliceSitePosVecSize == 0)
		{
			...
			tmpJunc_acceptorBranchingRatio = (double)tmpJunc_supportNum / (double)(tmpJunc_supportNum + tmpJunc_contiguousReadCount_acceptorSite); //1.0;
		else
		{
			int totalOtherJuncSupNum = 0;
		...
		branchingRatioJunc_ofs << indexInfo->returnChrNameStr(tmpJunc_chrNameInt) << "\t" << tmpJunc_donerEndPos << "\t" << tmpJunc_acceptorStartPos 
			...
			<< "\t" << tmpJunc_flankString_ref << "\t" << tmpJunc_flankString_hap1 << "\t" << tmpJunc_flankString_hap2 
			<< "\t" << tmpJunc_anchorSize_doner << "\t" << tmpJunc_anchorSize_acceptor << endl;;
	}
