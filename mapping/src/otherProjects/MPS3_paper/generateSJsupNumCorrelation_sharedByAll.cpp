...
int main(int argc, char** argv)
{
	...
	GTjuncHash->compared2otherAlignInferJuncHashVecAndReturnSJsupNumVec(
		juncHashVec, chrNameIntVecInGroundTruthJuncHash, donerSpliceSiteVecInGroundTruthJuncHash, acceptorSpliceSiteVecInGroundTruthJuncHash, 
		supNumInGroundTruthJuncHash, supNumVecInJuncHashVec2compare, sharedByAllOrNotBoolVec, min_intron_size, max_intron_size, 
		totalJuncNum_groundTruth, validJuncNum_groundTruth, invalidJuncNum_groundTruth, indexInfo);
	...
	for(int tmp = 0; tmp < validJuncNum_groundTruth; tmp++)
	{
		...
		{	
			...
			SJsupNum_ofs << tmpChrNameStr << "\t" << tmpDonerSpliceSite << "\t" << tmpAcceptorSpliceSite << "\t" << tmpSupNum;
			SJsupNum_sharedByAll_ofs << tmpChrNameStr << "\t" << tmpDonerSpliceSite << "\t" << tmpAcceptorSpliceSite << "\t" << tmpSupNum;
			SJsupNum_sharedByAll_log10_ofs << tmpChrNameStr << "\t" << tmpDonerSpliceSite << "\t" << tmpAcceptorSpliceSite << "\t" << tmpSupNum_log10;
	...
	for(int tmp = 0; tmp < toCompareAlignerSJnum; tmp++)
	{
		...
		for(int tmpJunc = 0; tmpJunc < validJuncNum_groundTruth; tmpJunc++)
		{
			...
			if(sharedByAllOrNot_bool)
			{
				...
				tmpSJsupNum_tmpAligner_ofs << tmpChrNameStr << "\t" << tmpDonerSpliceSite << "\t" << tmpAcceptorSpliceSite << "\t" << tmpSupNum_groundTruth 
					<< "\t" << tmpSupNum_groundTruth_log10 << "\t" << tmpSupNum_tmpAligner_log10 << "\t" << tmpAlignerName << endl;
			}
		}
