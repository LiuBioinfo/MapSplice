...
void cigarString2jumpCodeVec(string& jumpCodeStr, vector<Jump_Code>& cigarStringJumpCodeVec)
{
	...
	string candidateJumpCodeType = "SMNIDH";
	while(1)
	{
...
int getSoftOrHardClippedHeadLengthFromCigarString(string& tmpCigarStringStr)
{
	vector<Jump_Code> tmpJumpCodeVec;
	//cout << "cigarString " << cigarString << endl;
	cigarString2jumpCodeVec(tmpCigarStringStr, tmpJumpCodeVec);	
	if((tmpJumpCodeVec[0].type == "S")||(tmpJumpCodeVec[0].type == "H"))
		return tmpJumpCodeVec[0].len;
	else
		return 0; 
}	

int getSoftOrHardClippedTailLengthFromCigarString(string& tmpCigarStringStr)
{
	vector<Jump_Code> tmpJumpCodeVec;
	//cout << "cigarString " << cigarString << endl;
	cigarString2jumpCodeVec(tmpCigarStringStr, tmpJumpCodeVec);
	int tmpJumpCodeVecSize = tmpJumpCodeVec.size();
	if((tmpJumpCodeVec[tmpJumpCodeVecSize-1].type == "S")||(tmpJumpCodeVec[tmpJumpCodeVecSize-1].type == "H"))
...
int main(int argc, char** argv)
{
	...
	string outputUnmappedBaseNumPercFile_paired = outputFolderStr + "UnmappedBaseNumPerc_paired.txt";
	ofstream unmappedBaseNumPerc_ofs_paired(outputUnmappedBaseNumPercFile_paired.c_str());	
	...
	vector<int> mappedBaseNumVec_primaryAlignment_paired;
	for(int tmp = 0; tmp < fixedReadLength; tmp++)
		mappedBaseNumVec_primaryAlignment_paired.push_back(0);
	...
	while(!(sam_ifs.eof()))
	{
		...
		else
		{
			...
			{
				int softOrHardClippedHeadLength = getSoftOrHardClippedHeadLengthFromCigarString(cigarString);
				int softOrHardClippedTailLength = getSoftOrHardClippedTailLengthFromCigarString(cigarString);
				int startMappedBaseLocInRead = softOrHardClippedHeadLength + 1;
				int endMappedBaseLocInRead = fixedReadLength - softOrHardClippedTailLength;
				...
				if(paired_bool)
				{
					...
					for(int tmpBaseLoc = startMappedBaseLocInRead; 
							tmpBaseLoc <= endMappedBaseLocInRead; tmpBaseLoc++)
						mappedBaseNumVec_primaryAlignment_paired[tmpBaseLoc-1] ++;
				}
				else
				{
	...
	double tmpUnmappedBasePerc_sum = 0.0;
	for(int tmp = 0; tmp < fixedReadLength; tmp++)
	{
		...
		tmpUnmappedBasePerc_sum += tmpUnmappedBasePerc;
		unmappedBaseNumPerc_ofs << "Base[" << tmpBaseLoc << "]:\t" << tmpUnmappedBaseNum << "\t" << tmpUnmappedBasePerc << "\t" << methodName << endl;
	}
	double tmpUnmappedBasePerc_avg = tmpUnmappedBasePerc_sum / (double)fixedReadLength;
	alignmentTypeNumPerc_ofs << endl << "Total unmapped base num perc:\t" << tmpUnmappedBasePerc_avg << "\t" << (100.00 - tmpUnmappedBasePerc_avg) << endl;

	double tmpUnmappedBasePerc_sum_paired = 0.0;
	for(int tmp = 0; tmp < fixedReadLength; tmp++)
	{
		int tmpBaseLoc = tmp + 1;
		int tmpMappedBaseNum_paired = mappedBaseNumVec_primaryAlignment_paired[tmp];
		int tmpUnmappedBaseNum_paired = totalAlignmentNumber - mappedBaseNumVec_primaryAlignment_paired[tmp];
		double tmpUnmappedBasePerc_paired = ((double)tmpUnmappedBaseNum_paired/(double)totalAlignmentNumber) * 100;
		tmpUnmappedBasePerc_sum_paired += tmpUnmappedBasePerc_paired;
		unmappedBaseNumPerc_ofs_paired << "Base[" << tmpBaseLoc << "]:\t" << tmpUnmappedBaseNum_paired << "\t" << tmpUnmappedBasePerc_paired << "\t" << methodName << endl;
	}
	double tmpUnmappedBasePerc_avg_paired = tmpUnmappedBasePerc_sum_paired / (double)fixedReadLength;
	alignmentTypeNumPerc_ofs << endl << "Total unmapped base num perc (paired):\t" << tmpUnmappedBasePerc_avg_paired << "\t" << (100.00 - tmpUnmappedBasePerc_avg_paired) << endl;

	sam_ifs.close();
	alignmentTypeNumPerc_ofs.close();
	unmappedBaseNumPerc_ofs.close();
	unmappedBaseNumPerc_ofs_paired.close();
	return 0;
}