...
{
	bool segMap2SNPmer_phase2Only_bool = true;
	cout << "segMap2SNPmer_phase2Only_bool: " << segMap2SNPmer_phase2Only_bool << endl;
    ...
    string chromDirStr = optionInfo->chromsome_file_path_prefix; //argv[8];
	...
	if(!segMap2SNPmer_phase2Only_bool)	
...
{			
	...
	if(DoRemappingOnUnmapEndReadsBool)
	{
		...
		for(tmpTurn = 0; /*tmpTurn < TurnNum*/; tmpTurn++)
		{
			...
			nowtime = time(NULL);
			local = localtime(&nowtime);
			runtime_log_ofs << endl << "[" << asctime(local) << "finish fixing oneEndUnmapped, turn: " << tmpTurn+1 << endl;// << endl;
			runtime_log_ofs << "start to output ... turn: " << tmpTurn+1 << endl;
...
{	
				...
				if(SE_or_PE_bool)
				{
				    ...
				    if(alignmentScore_tooLow)
				    	...
				    	seAlignSamVec_incomplete[tmpOpenMP] = peAlignInfo->getSAMformatForFinalAlignment_SE(peReadInfo, fasta_or_fastq_bool);
				}				
				else
				{	
					bool pairExistsBool = peAlignInfo->finalPairExistsBool();									
					...
					if(pairExistsBool) // some pair exists, all completed, print out paired SAM info
					{
						...
						if(align_lowScore_bool)
						{
							...
							peAlignSamVec_pair_lowScore[tmpOpenMP] = peAlignInfo->getSAMformatForBothEndsUnmapped(peReadInfo, fasta_or_fastq_bool);
						}
						else if(allFinalPairAlignmentCompleteBool)
							peAlignSamVec_complete_pair[tmpOpenMP] = peAlignInfo->getSAMformatForFinalPair_secondaryOrNot(peReadInfo, fasta_or_fastq_bool, multiMapSeg_maxLength);
						else
							peAlignSamVec_incomplete_pair[tmpOpenMP] = peAlignInfo->getSAMformatForFinalPair_secondaryOrNot(peReadInfo, fasta_or_fastq_bool, multiMapSeg_maxLength);		
					}
					else //if((!pairExistsBool) && (allAlignmentCompleteBool)) // no pair exists, all complete, print out original SAM info
					{
						if(!outputUnpairedSAM_bool)
						{
							...
							if(allUnpairAlignmentCompleteBool)
								peAlignSamVec_complete_unpair[tmpOpenMP] 
									= peAlignInfo->getSAMformatForUnpairedAlignments_secondaryOrNot(peReadInfo, fasta_or_fastq_bool);
							else
								peAlignSamVec_incomplete_unpair[tmpOpenMP] 
									= peAlignInfo->getSAMformatForUnpairedAlignments_secondaryOrNot(peReadInfo, fasta_or_fastq_bool);
						}
			...
			fixHeadTail_overallRunning_cost = fixHeadTail_overallRunning_cost + fixHeadTail_overallRunning_end - fixHeadTail_overallRunning_begin;
			#endif

			nowtime = time(NULL);
			local = localtime(&nowtime);
			runtime_log_ofs << endl << "[" << asctime(local) << "finish fixing Head/Tail, turn: " << tmpTurn+1 << endl;// << endl;
			runtime_log_ofs << endl << "[" << asctime(local) << "start to output ... turn: " << tmpTurn+1 << endl;
