...
public:
	...
	int return_chrNameIndexArray_vecSize()
	{
		return chrNameIndexArray.size();
	}

	int return_chrNameIndexArray_vecElem(int tmp)
	{
		return chrNameIndexArray[tmp];
	}

	int return_chrNameIndexIntervalSize()
	{
		return chrNameIndexIntervalSize;
	}

	unsigned int return_null_num()
	{
		return null_num;
	}

	unsigned int return_indexSize()
	{
		return indexSize;
	}

	int return_secondLevelIndexPartsNumSum()
	{
		return secondLevelIndexPartsNumSum;
	}

	int return_secondLevelIndexNormalSize()
	{
		return secondLevelIndexNormalSize;
	}

	int return_secondLevelIndexPartsNum_vecSize()
	{
		return secondLevelIndexPartsNum.size();
	}

	int return_secondLevelIndexPartsNum_vecElem(int tmp)
	{
		return secondLevelIndexPartsNum[tmp];
	}

	string return_chromString()
	{
		return chromString;
	}

	unsigned int return_genomeLength()
	{
		return genomeLength;
	}
	...
	int return_chrNameStr_vecSize()
	{
		return chrNameStr.size();
	}

	string return_chrNameStr_vecElem(int tmp)
	{
		return chrNameStr[tmp];
	}

	int return_chromLength_vecSize()
	{
		return chromLength.size();
	}

	int return_chromLength_vecElem(int tmp)
	{
		return chromLength[tmp];
	}

	int return_chromStr_vecSize()
	{
		return chromStr.size();
	}

	string return_chromStr_vecElem(int tmp)
	{
		return chromStr[tmp];
	}

	int return_chrEndPosInGenome_vecSize()
	{
		return chrEndPosInGenome.size();
	}

	unsigned int return_chrEndPosInGenome_vecElem(int tmp)
	{
		return chrEndPosInGenome[tmp];
	}	
	...
	void cpIndex(Index_Info* index2cp)
	{
		chromString = index2cp->return_chromString();
		genomeLength = index2cp->return_genomeLength();
		chromNum = index2cp->return_chromNum();
		int chromNameStr_vecSize = index2cp->return_chrNameStr_vecSize();
		for(int tmp = 0; tmp < chromNameStr_vecSize; tmp++)
			chrNameStr.push_back(index2cp->return_chrNameStr_vecElem(tmp));
		int chromLength_vecSize = index2cp->return_chromLength_vecSize();
		for(int tmp = 0; tmp < chromLength_vecSize; tmp++)
			chromLength.push_back(index2cp->return_chromLength_vecElem(tmp));
		int chromStr_vecSize = index2cp->return_chromStr_vecSize();
		for(int tmp = 0; tmp < chromStr_vecSize; tmp++)
			chromStr.push_back(index2cp->return_chromStr_vecElem(tmp));
		int chrEndPosInGenome_vecSize = index2cp->return_chrEndPosInGenome_vecSize();
		for(int tmp = 0; tmp < chrEndPosInGenome_vecSize; tmp++)
			chrEndPosInGenome.push_back(index2cp->return_chrEndPosInGenome_vecElem(tmp));
		secondLevelIndexNormalSize = index2cp->return_secondLevelIndexNormalSize();
		int secondLevelIndexPartsNum_vecSize = index2cp->return_secondLevelIndexPartsNum_vecSize();
		for(int tmp = 0; tmp < secondLevelIndexPartsNum_vecSize; tmp++)
			secondLevelIndexPartsNum.push_back(index2cp->return_secondLevelIndexPartsNum_vecElem(tmp));
		secondLevelIndexPartsNumSum = index2cp->return_secondLevelIndexPartsNumSum();
		null_num = index2cp->return_null_num();
		indexSize = index2cp->return_indexSize();
		int chrNameIndexArray_vecSize = index2cp->return_chrNameIndexArray_vecSize();
		for(int tmp = 0; tmp < chrNameIndexArray_vecSize; tmp++)
			chrNameIndexArray.push_back(index2cp->return_chrNameIndexArray_vecElem(tmp));
		chrNameIndexIntervalSize = index2cp->return_chrNameIndexIntervalSize();

		for(map<string, int>::iterator tmpIter = (index2cp->chrNameMap).begin(); tmpIter != (index2cp->chrNameMap).end(); tmpIter++)
			chrNameMap.insert(pair<string,int>(tmpIter->first, tmpIter->second));
		for(set<int>::iterator tmpIter = (index2cp->invalidSecondLevelIndexNOset).begin(); tmpIter != (index2cp->invalidSecondLevelIndexNOset).end(); tmpIter++)
			invalidSecondLevelIndexNOset.insert(*tmpIter);
		for(map<int, set<int> >::iterator tmpMapIter = (index2cp->chrNameIndexArrayMap).begin(); tmpMapIter != (index2cp->chrNameIndexArrayMap).end(); tmpMapIter++)
		{
			int tmpChrNameIndex = tmpMapIter->first;
			set<int> tmpIntSet;
			for(set<int>::iterator tmpSetIter = (tmpMapIter->second).begin(); tmpSetIter != (tmpMapIter->second).end(); tmpSetIter++)
				tmpIntSet.insert(*tmpSetIter);
			chrNameIndexArrayMap.insert(pair<int, set<int> >(tmpChrNameIndex, tmpIntSet));
		}
	}
	...
	void maskSNPwithNinChromStr(string& SNPfilePath)
	{
		cout << "start to mask SNPs with Ns in ChromStr ! " << endl;
		...
		while(!SNP_ifs.eof())
		{
			...
			{
				if((tmpPos < 1)||(tmpPos > this->returnChromLength(tmpChrNameInt)))
					tmpSNP_num_invalid ++;
				else
				{
					tmpSNP_num_valid ++;
					chromStr[tmpChrNameInt].replace(tmpPos - 1, 1, "N");
				}
			}
		}
		cout << "end of masking SNPs with Ns in ChromStr ! " << endl;
	}
	...
	void printEachChromStr2oneFile(string& outputDir, int chr_seq_fa_line_length)
	{
		outputDir += "/";
		...
		for(int tmp = 0; tmp < chromNum; tmp++)
		{
			string tmpChrName = this->returnChrNameStr(tmp);
			string tmpChrFile = outputDir + tmpChrName + ".fa";
			ofstream tmpChr_ofs(tmpChrFile.c_str());
			// print id
			tmpChr_ofs << ">" << tmpChrName << endl;
			int tmpChrLength = this->returnChromLength(tmp);
			if(tmp == chromNum - 1)
				tmpChrLength = tmpChrLength - 1;
			int tmpFullBaseLineNum = tmpChrLength / chr_seq_fa_line_length;
			bool lastLineFullBaseOrNot = (tmpFullBaseLineNum * chr_seq_fa_line_length == tmpChrLength);				
			// print all fullBase lines
			for(int tmpLine = 0; tmpLine < tmpFullBaseLineNum; tmpLine++)
				tmpChr_ofs << this->returnChromStrSubstr(tmp, tmpLine * chr_seq_fa_line_length + 1, chr_seq_fa_line_length) << endl;		
			// print the last line
			if(!lastLineFullBaseOrNot)
				tmpChr_ofs << this->returnChromStrSubstr(tmp, tmpFullBaseLineNum * chr_seq_fa_line_length + 1, 
					tmpChrLength - tmpFullBaseLineNum * chr_seq_fa_line_length) << endl;
			tmpChr_ofs.close();
		}
	}
