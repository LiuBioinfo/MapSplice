...
private:
	...
	vector< int > seAlignVec_final_Rcm;
 	// ******************************************************* //	
public:
	bool return_validForFusionPostAnalysis_bool()
	{
	...
	string returnSamStr_SE_uniqAlign()
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
		...
		//tmp_clippedSeg_forOrRevMap_bool = Nor_or_Rcm_bool;
		if(Nor_or_Rcm_bool)
		{
			int tmpNor_index = seAlignVec_final_Nor[0];
			string tmpNor_chrName = norAlignmentInfo_PE_1[tmpNor_index]->returnAlignChromName();
			int tmpNor_chrPos = norAlignmentInfo_PE_1[tmpNor_index]->returnAlignChromPos();
			string tmpNor_cigarString = "";
			for(int tmp = 0; tmp < (norAlignmentInfo_PE_1[tmpNor_index]->cigarStringJumpCode).size(); tmp++)
			{
				tmpNor_cigarString += int_to_str((norAlignmentInfo_PE_1[tmpNor_index]->cigarStringJumpCode)[tmp].len);
				tmpNor_cigarString += (norAlignmentInfo_PE_1[tmpNor_index]->cigarStringJumpCode)[tmp].type;
			}
			string tmpSamStr = tmpNor_chrName + "\t" + int_to_str(tmpNor_chrPos) + "\t" + tmpNor_cigarString + "\tNor"; 
			return tmpSamStr;
		}
		else
		{
			int tmpRcm_index = seAlignVec_final_Rcm[0];
			string tmpRcm_chrName = rcmAlignmentInfo_PE_1[tmpRcm_index]->returnAlignChromName();
			int tmpRcm_chrPos = rcmAlignmentInfo_PE_1[tmpRcm_index]->returnAlignChromPos();
			string tmpRcm_cigarString = "";
			for(int tmp = 0; tmp < (rcmAlignmentInfo_PE_1[tmpRcm_index]->cigarStringJumpCode).size(); tmp++)
			{
				tmpRcm_cigarString += int_to_str((rcmAlignmentInfo_PE_1[tmpRcm_index]->cigarStringJumpCode)[tmp].len);
				tmpRcm_cigarString += (rcmAlignmentInfo_PE_1[tmpRcm_index]->cigarStringJumpCode)[tmp].type;
			}
			string tmpSamStr = tmpRcm_chrName + "\t" + int_to_str(tmpRcm_chrPos) + "\t" + tmpRcm_cigarString + "\tNor"; 
			return tmpSamStr;
		}			
	}

	void initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign(bool& tmp_clippedSeg_forOrRevMap_bool,
	 	int& tmp_chrNameInt_clippedSeg, int& tmp_chrPos_clippedSeg, vector<Jump_Code>& tmp_jumpCodeVec_clippedSeg, 
	 	Index_Info* indexInfo)
	{
		...
		{
			int tmpNor_index = seAlignVec_final_Nor[0];
			string tmp_chrNameStr_clippedSeg = norAlignmentInfo_PE_1[tmpNor_index]->returnAlignChromName();
			tmp_chrNameInt_clippedSeg = indexInfo->convertStringToInt(tmp_chrNameStr_clippedSeg);
			tmp_chrPos_clippedSeg = norAlignmentInfo_PE_1[tmpNor_index]->returnAlignChromPos();
			norAlignmentInfo_PE_1[tmpNor_index]->pushBackJumpCodeVec2target(tmp_jumpCodeVec_clippedSeg);
		}
		else
		{
			int tmpRcm_index = seAlignVec_final_Rcm[0];
			string tmp_chrNameStr_clippedSeg = rcmAlignmentInfo_PE_1[tmpRcm_index]->returnAlignChromName();
			tmp_chrNameInt_clippedSeg = indexInfo->convertStringToInt(tmp_chrNameStr_clippedSeg);
			tmp_chrPos_clippedSeg = rcmAlignmentInfo_PE_1[tmpRcm_index]->returnAlignChromPos();
			rcmAlignmentInfo_PE_1[tmpRcm_index]->pushBackJumpCodeVec2target(tmp_jumpCodeVec_clippedSeg);
		}
	}

	string returnFirstJumpCodeType_SE_uniqAlign()
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
		...
		{
			int tmpNor_index = seAlignVec_final_Nor[0];
			return norAlignmentInfo_PE_1[tmpNor_index]->returnFirstJumpCodeType();
		}
		else
		{
			int tmpRcm_index = seAlignVec_final_Rcm[0];
			return rcmAlignmentInfo_PE_1[tmpRcm_index]->returnFirstJumpCodeType();
		}					
	}

	string returnLastJumpCodeType_SE_uniqAlign()
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
		...
		{
			int tmpNor_index = seAlignVec_final_Nor[0];
			return norAlignmentInfo_PE_1[tmpNor_index]->returnLastJumpCodeType();
		}
		else
		{
			int tmpRcm_index = seAlignVec_final_Rcm[0];
			return rcmAlignmentInfo_PE_1[tmpRcm_index]->returnLastJumpCodeType();
		}		
	}	

	bool returnForMapOrRcmMap_SE_uniqAlign()
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
		...
		bool tmp_Nor_or_Rcm_bool = (seAlignVec_final_Nor.size() == 1);
		return tmp_Nor_or_Rcm_bool;
	}

	int returnChrNameInt_SE_uniqAlign(Index_Info* indexInfo)
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
		...
		{
			int tmpNor_index = seAlignVec_final_Nor[0];
			string tmpChrName = norAlignmentInfo_PE_1[tmpNor_index]->returnAlignChromName();
			return indexInfo->convertStringToInt(tmpChrName);
		}
		else
		{
			int tmpRcm_index = seAlignVec_final_Rcm[0];
			string tmpChrName = rcmAlignmentInfo_PE_1[tmpRcm_index]->returnAlignChromName();
			return indexInfo->convertStringToInt(tmpChrName);
		}			
	}

	int returnChrPos_SE_uniqAlign()
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
	...
	void pushBackJumpCodeVec2target_SE_uniqAlign(vector<Jump_Code>& targetJumpCodeVec)
	{
		if(seAlignVec_final_Nor.size() + seAlignVec_final_Rcm.size() != 1)
		{
			cout << "initiate_clippedSegSamForFusionPostAnalysis_SE_uniqAlign invalid ! " << endl;
		...
		{
			int tmpNor_index = seAlignVec_final_Nor[0];
			norAlignmentInfo_PE_1[tmpNor_index]->pushBackJumpCodeVec2target(targetJumpCodeVec);
		}
		else
		{
			int tmpRcm_index = seAlignVec_final_Rcm[0];
			rcmAlignmentInfo_PE_1[tmpRcm_index]->pushBackJumpCodeVec2target(targetJumpCodeVec);
		}
	}

	// int getEndLocInReadOfSpecificJumpCode(vector<Jump_Code>& tmpCigarStringJumpCodeVec, int jumpCodeIndex)
	// {
	// 	if(jumpCodeIndex < 0)
	// 		return 0;
	...
	// 	{
	// 		string tmpJumpCodeType = tmpCigarStringJumpCodeVec[tmpIndex].type;
	// 		int tmpJumpCodeLength = tmpCigarStringJumpCodeVec[tmpIndex].len;
	...
	// int getEndPosOfSpecificJumpCode(int tmpStartPos, vector<Jump_Code>& tmpCigarStringJumpCodeVec, int jumpCodeIndex)
	// {
	// 	int tmpEndPos = 0;
	// 	for(int tmpIndex = 0; tmpIndex <= jumpCodeIndex; tmpIndex++)
	// 	{
	...
	// 		}								
	// 	}
	// 	return (tmpEndPos + tmpStartPos-1);
	// }

	// void cigarString2jumpCodeVec(string& tmpJumpCodeStr, vector<Jump_Code>& tmpCigarStringJumpCodeVec)
	// {
	...
	// 			tmpJumpCodeStr.find_first_of(candidateJumpCodeType, jumpCodeStartPosInCigarStr);
	// 		if(jumpCodeEndPosInCigarStr == tmpJumpCodeStr.npos)
	// 			{break;}
	// 		else
	// 		{
	// 			tmpJumpCodeLength = 
	// 				atoi((tmpJumpCodeStr.substr(jumpCodeStartPosInCigarStr, jumpCodeEndPosInCigarStr - jumpCodeStartPosInCigarStr)).c_str());
	// 			tmpJumpCodeType = tmpJumpCodeStr.substr(jumpCodeEndPosInCigarStr, 1);
	// 			tmpCigarStringJumpCodeVec.push_back(Jump_Code(tmpJumpCodeLength, tmpJumpCodeType));
	// 			jumpCodeStartPosInCigarStr = jumpCodeEndPosInCigarStr + 1;
	// 		}
	// 	}
	// }

	// void generateExonLocInReadPosInChr(int tmpStartPos, vector<Jump_Code>& tmpCigarStringJumpCodeVec, 
	// 	vector<int>& tmpEndLocVecInRead, vector<int>& tmpEndPosVecInChr, vector<int>& tmpLenVec);
	// {
	// 	for(int tmp = 0; tmp < tmpCigarStringJumpCodeVec.size(); tmp ++)
	// 	{
	// 		string tmpJumpCodeType = tmpCigarStringJumpCodeVec[tmp].type;
	// 		int tmpJumpCodeLength = tmpCigarStringJumpCodeVec[tmp].len;
	// 		int tmpJumpCodeIndex = tmp;
	// 		if(tmpJumpCodeType == "M")
	// 		{
	// 			int tmpEndLocInRead = getEndLocInReadOfSpecificJumpCode(tmpCigarStringJumpCodeVec, tmpJumpCodeIndex);
	// 			int tmpEndPosInChr = getEndPosOfSpecificJumpCode(tmpStartPos, tmpCigarStringJumpCodeVec, tmpJumpCodeIndex);
	// 			tmpEndLocVecInRead.push_back(tmpEndLocInRead);
	// 			tmpEndPosVecInChr.push_back(tmpEndPosInChr);
	// 			tmpLenVec.push_back(tmpJumpCodeLength);
	// 		}
	// 	}
	// }

	int returnMinMappedBaseForEachRead_toLearnSNP_withReadLength(int tmpReadLength)
	{
		if(tmpReadLength > 105)
		{
			return 100;
		}
		else
			return tmpReadLength - 5;
	}

	void collectMismatch(LearnedCandiSNPhash_Info_Vec& mismatchHashInfoVec, int indexInMismatchHashInfoVec, Index_Info* indexInfo,
		int tmpReadLength_end1, int tmpReadLength_end2)
	{
		int learnSNP_candiMapOptionNum_max = LEARNSNP_CANDIMAPOPTIONNUM_MAX;
		//bool learnSNP_completePairOnly_bool = LEARNSNP_COMPLETEPAIRONLY_BOOL;
		int learnSNP_mismatchNum_max = LEARNSNP_MISMATCHNUM_MAX;
		int minMappedBaseForEachRead_end1 = this->returnMinMappedBaseForEachRead_toLearnSNP_withReadLength(tmpReadLength_end1);
		int minMappedBaseForEachRead_end2 = this->returnMinMappedBaseForEachRead_toLearnSNP_withReadLength(tmpReadLength_end2);
		int learnSNP_mappedBaseForEachRead_min_end1 = minMappedBaseForEachRead_end1;
		int learnSNP_mappedBaseForEachRead_min_end2 = minMappedBaseForEachRead_end2;
		int learnSNP_distance2exonBoundary_min = LEARNSNP_DISTANCE2EXONBOUNDARY_MIN;		
		int learnSNP_segLen_min = learnSNP_distance2exonBoundary_min * 2 + 1;
		
		int finalAlignPair_Nor1Rcm2_size = finalAlignPair_Nor1Rcm2.size();
		int finalAlignPair_Nor2Rcm1_size = finalAlignPair_Nor2Rcm1.size();
		if((finalAlignPair_Nor1Rcm2_size + finalAlignPair_Nor2Rcm1_size > learnSNP_candiMapOptionNum_max)
			|| (finalAlignPair_Nor1Rcm2_size + finalAlignPair_Nor2Rcm1_size == 0))
			return;
		else
		{
			// if(learnSNP_completePairOnly_bool) // check complete or not, if necessary
			// {
			// 	bool tmpAllAlignmentCompleteBool = this->allAlignmentInFinalPairCompleted();
			// 	if(!tmpAllAlignmentCompleteBool)
			// 		return;
			// }
			if(finalAlignPair_Nor1Rcm2_size == 1)
			{
				int tmpNorIndex = finalAlignPair_Nor1Rcm2[0].first;
				int tmpRcmIndex = finalAlignPair_Nor1Rcm2[0].second;
				int tmpMismatchNum_Nor = norAlignmentInfo_PE_1[tmpNorIndex]->returnMismatchNum();
				int tmpMismatchNum_Rcm = rcmAlignmentInfo_PE_2[tmpRcmIndex]->returnMismatchNum();
				int tmpMappedBaseNum_end1 = norAlignmentInfo_PE_1[tmpNorIndex]->returnMappedBaseNum();
				int tmpMappedBaseNum_end2 = rcmAlignmentInfo_PE_2[tmpRcmIndex]->returnMappedBaseNum();
				if((tmpMismatchNum_Nor <= learnSNP_mismatchNum_max)&&(tmpMappedBaseNum_end1 >= learnSNP_mappedBaseForEachRead_min_end1))
					norAlignmentInfo_PE_1[tmpNorIndex]->collectMismatch(learnSNP_distance2exonBoundary_min, 
						learnSNP_segLen_min, mismatchHashInfoVec, indexInMismatchHashInfoVec, indexInfo);
				if((tmpMismatchNum_Rcm <= learnSNP_mismatchNum_max)&&(tmpMappedBaseNum_end2 >= learnSNP_mappedBaseForEachRead_min_end2))
					rcmAlignmentInfo_PE_2[tmpRcmIndex]->collectMismatch(learnSNP_distance2exonBoundary_min, 
						learnSNP_segLen_min, mismatchHashInfoVec, indexInMismatchHashInfoVec, indexInfo);
			}
			else // finalAlignPair_Nor2Rcm1_size == 1
			{
				int tmpNorIndex = finalAlignPair_Nor2Rcm1[0].first;
				int tmpRcmIndex = finalAlignPair_Nor2Rcm1[0].second;
				int tmpMismatchNum_Nor = norAlignmentInfo_PE_2[tmpNorIndex]->returnMismatchNum();
				int tmpMismatchNum_Rcm = rcmAlignmentInfo_PE_1[tmpRcmIndex]->returnMismatchNum();			
				int tmpMappedBaseNum_end1 = rcmAlignmentInfo_PE_1[tmpRcmIndex]->returnMappedBaseNum();
				int tmpMappedBaseNum_end2 = norAlignmentInfo_PE_2[tmpNorIndex]->returnMappedBaseNum();
				if((tmpMismatchNum_Nor <= learnSNP_mismatchNum_max)&&(tmpMappedBaseNum_end2 >= learnSNP_mappedBaseForEachRead_min_end2))
					norAlignmentInfo_PE_2[tmpNorIndex]->collectMismatch(learnSNP_distance2exonBoundary_min, 
						learnSNP_segLen_min, mismatchHashInfoVec, indexInMismatchHashInfoVec, indexInfo);
				if((tmpMismatchNum_Rcm <= learnSNP_mismatchNum_max)&&(tmpMappedBaseNum_end1 >= learnSNP_mappedBaseForEachRead_min_end1))
					rcmAlignmentInfo_PE_1[tmpRcmIndex]->collectMismatch(learnSNP_distance2exonBoundary_min, 
						learnSNP_segLen_min, mismatchHashInfoVec, indexInMismatchHashInfoVec, indexInfo);
			}
		}
	}
	...
	void pairingAlignment2OriPair()
	{
		...
		this->getMismatchPosInChrVecForEveryAlignment();
		...
		int tmpOpenMP, int threadNO, Stats_Info* statsInfo, bool FastaOrFastq, int multiMapSeg_maxLength)
	{
		...
		if(pairExistsBool) // some pair exists
		{
			...
			{
				if(!Do_Phase1_Only)
					resultArray->insert_PeAlignInfoStrVec_inCompletePair(this->getTmpAlignInfoForFinalPair(readInfo.returnReadName_1(), readInfo.returnReadName_2(), 
						readInfo.returnReadSeq_1(), readInfo.returnReadSeq_2(), readInfo.returnReadQual_1(), readInfo.returnReadQual_2(), FastaOrFastq), tmpOpenMP);
				resultArray->insert_PeAlignSamStrVec_inCompletePair(this->getSAMformatForFinalPair_secondaryOrNot(readInfo, FastaOrFastq), tmpOpenMP);
			}
		}
		else // no pair exists: 1.one end unmapped; 2. both ends unmapped
		{
			...
			{
				if(Do_Phase1_Only)
					resultArray->insert_PeAlignInfoStrVec_oneEndUnmapped(this->getSAMformatForBothEndsUnmapped(readInfo, FastaOrFastq), tmpOpenMP);
				else
					resultArray->insert_PeAlignInfoStrVec_oneEndUnmapped(this->getTmpAlignInfo(readInfo.returnReadName_1(), readInfo.returnReadName_2(), 
						readInfo.returnReadSeq_1(), readInfo.returnReadSeq_2(), readInfo.returnReadQual_1(), readInfo.returnReadQual_2(), FastaOrFastq), tmpOpenMP);
			}
			else if(mappedToRepeatRegionBool)
	...
	void getMismatchPosInChrVecForEveryAlignment_SE()
	{
		for(int tmp = 0; tmp < norAlignmentInfo_PE_1.size(); tmp++)
		{
			norAlignmentInfo_PE_1[tmp]->generateMismatchPosInChrVec();
		}
		for(int tmp = 0; tmp < rcmAlignmentInfo_PE_1.size(); tmp++)
		{
			rcmAlignmentInfo_PE_1[tmp]->generateMismatchPosInChrVec();
		}		
	}

	void getMismatchPosInChrVecForEveryAlignment_end1()
	{
		this->getMismatchPosInChrVecForEveryAlignment_SE();
	}

	void getMismatchPosInChrVecForEveryAlignment_end2()
	{
		for(int tmp = 0; tmp < norAlignmentInfo_PE_2.size(); tmp++)
		{
			norAlignmentInfo_PE_2[tmp]->generateMismatchPosInChrVec();
		}
		for(int tmp = 0; tmp < rcmAlignmentInfo_PE_2.size(); tmp++)
		{
			rcmAlignmentInfo_PE_2[tmp]->generateMismatchPosInChrVec();
		}
	}	
	...
	void getMismatchPosInChrVecForEveryAlignment()
	{
		for(int tmp = 0; tmp < norAlignmentInfo_PE_1.size(); tmp++)
		{
			norAlignmentInfo_PE_1[tmp]->generateMismatchPosInChrVec();
	...
	string getSAMformatForUnpairedAlignments_secondaryOrNot_allCases(
		PE_Read_Info& peReadInfo, bool FastaOrFastq)
	{
		return this->getSAMformatForUnpairedAlignments_secondaryOrNot_allCases(
			peReadInfo, FastaOrFastq, 0);
	}
