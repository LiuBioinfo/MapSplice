...
private:
	...
	vector<int> mismatchPosInChrVec; // for obtaining self learned SNPs, which would be used to apply segment map 2 SNPmers.
public:
	vector<Jump_Code> cigarStringJumpCode;
	vector< pair< int, SpliceJunction_Alignment > > spliceJunctionVec; // <posInRead, SpliceJunction>
	
	string returnFirstJumpCodeType()
	{
		return cigarStringJumpCode[0].type;
	}

	string returnLastJumpCodeType()
	{
		return cigarStringJumpCode[cigarStringJumpCode.size()-1].type;
	}	

	void pushBackJumpCodeVec2target(vector<Jump_Code>& targetJumpCodeVec)
	{
		for(int tmp = 0; tmp < cigarStringJumpCode.size(); tmp++)
			targetJumpCodeVec.push_back(cigarStringJumpCode[tmp]);
	}

	void collectMismatch(int learnSNP_distance2exonBoundary_min, int learnSNP_segLen_min, 
		LearnedCandiSNPhash_Info_Vec& mismatchHashInfoVec, int indexInMismatchHashInfoVec, Index_Info* indexInfo)
	{
		int tmpChrNameInt = indexInfo->convertStringToInt(alignChromName);
		vector<int> tmpEndLocVecInRead;
		vector<int> tmpEndPosVecInChr;
		vector<int> tmpLenVec;
		generateExonLocInReadPosInChr(//alignChromPos, cigarStringJumpCode, 
			tmpEndLocVecInRead, tmpEndPosVecInChr, tmpLenVec);		
		int tmpExonNum = tmpLenVec.size();
		for(int tmpMismatchIndex = 0; tmpMismatchIndex < mismatchNum; tmpMismatchIndex ++)
		{
			int tmpMismatchLocInRead = mismatchPosVec[tmpMismatchIndex];
			for(int tmpExonIndex = 0; tmpExonIndex < tmpExonNum; tmpExonIndex++)
			{
				int tmpLen = tmpLenVec[tmpExonIndex];				
				if(tmpLen >= learnSNP_segLen_min)
				{
					int tmpEndLocInRead = tmpEndLocVecInRead[tmpExonIndex];
					int tmpStartLocInRead = tmpEndLocInRead - tmpLen + 1;
					if((tmpMismatchLocInRead >= tmpStartLocInRead + learnSNP_distance2exonBoundary_min)
						&&(tmpMismatchLocInRead <= tmpEndLocInRead - learnSNP_distance2exonBoundary_min))
					{
						char tmpMismatchChar = mismatchCharVec[tmpMismatchIndex];
						string tmpMismatchCharStr(1, tmpMismatchChar);
						int tmpMismatchPosInChr = mismatchPosInChrVec[tmpMismatchIndex];
						mismatchHashInfoVec.addMismatchInSam(tmpChrNameInt, tmpMismatchPosInChr, tmpMismatchCharStr, 
							indexInfo, indexInMismatchHashInfoVec);
					}
				}
			}
		}
	...
	void generateExonLocInReadPosInChr(vector<int>& endLocVecInRead, vector<int>& endPosVecInChr, vector<int>& lenVec)
	{
		for(int tmp = 0; tmp < cigarStringJumpCode.size(); tmp ++)
		{
			...
			{
				int tmpEndLocInRead = this->getEndLocInReadOfSpecificJumpCode(cigarStringJumpCode, tmpJumpCodeIndex);
				int tmpEndPosInChr = this->getEndPosOfSpecificJumpCode(alignChromPos, cigarStringJumpCode, tmpJumpCodeIndex);
	...
	void generateMismatchPosInChrVec()
	{
		...
		vector<int> tmpEndPosVecInChr;
		vector<int> tmpLenVec;
		this->generateExonLocInReadPosInChr(tmpEndLocVecInRead, tmpEndPosVecInChr, tmpLenVec);
		int tmpExonNum = tmpEndLocVecInRead.size();
		for(int tmpExonIndex = 0; tmpExonIndex < tmpExonNum; tmpExonIndex ++)
		{
			int tmpEndLocInRead = tmpEndLocVecInRead[tmpExonIndex];
			int tmpEndPosInChr = tmpEndPosVecInChr[tmpExonIndex];
			int tmpLen = tmpLenVec[tmpExonIndex];			
			int tmpStartLocInRead = tmpEndLocInRead - tmpLen + 1;
			int tmpStartPosInChr = tmpEndPosInChr - tmpLen + 1;
			for(int tmpMismatchIndex = 0; tmpMismatchIndex < mismatchPosVec.size(); tmpMismatchIndex ++)
			{
				int tmpMismatchLocInRead = mismatchPosVec[tmpMismatchIndex];
				if((tmpMismatchLocInRead >= tmpStartLocInRead)&&(tmpMismatchLocInRead <= tmpEndLocInRead))
				{
					int tmpDistanceToExonStart = tmpMismatchLocInRead - tmpStartLocInRead;
					int tmpMismatchPosInChr = tmpDistanceToExonStart + tmpStartPosInChr;
					mismatchPosInChrVec.push_back(tmpMismatchPosInChr);
				}
			}
		}
	}
		...
		bool SecondaryOrNot, bool fasta_or_fastq_bool)
	{
		...
		if(mismatchPosVec.size() > 0)
		{
			...
			samString = samString + "\tMG:Z:";
			for(int tmp = 0; tmp < mismatchPosInChrVec.size(); tmp++)
				samString = samString + int_to_str(mismatchPosInChrVec[tmp]) + ",";
		}
		return samString;
	}
