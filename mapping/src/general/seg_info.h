...
public:
	Seg_Info()
	{
		...
		for(int tmp = 0; tmp < SEGMENTNUM; tmp++)
			norSegmentUpdatedWithSNPorNotBool[tmp] = false;
		//#endif
	}
	...
	int getMaxSegLength()
	{
		...
		for(int tmp = 0; tmp < segmentNum; tmp++)
		{
			if(norSegmentLength[tmp] > tmpMaxLength)
				tmpMaxLength = norSegmentLength[tmp];
		}
		return tmpMaxLength;
	}

	int getMaxSegLengthAndHitNum(int& maxSegHitNum)
	{
		maxSegHitNum = 0;
		int tmpMaxLength = 0;		
		for(int tmp = 0; tmp < segmentNum; tmp++)
		{
			...
			{
				tmpMaxLength = norSegmentLength[tmp];
				maxSegHitNum = norSegmentAlignNum[tmp];
			}
		}
		return tmpMaxLength;
	}
	...
	{
		//cout << "start to getFirstLongSegNO " << endl;
		bool longSegExists = false;
		for(int tmpSegNO = 0; tmpSegNO < segmentNum; tmpSegNO++)
		{
			//cout << "norSegmentUpdatedWithSNPorNotBool[" << tmpSegNO << "]: " << norSegmentUpdatedWithSNPorNotBool[tmpSegNO] << endl; 
			//#ifdef PERSONALIZED_CHR_SEQ
	...
	bool mapMain_preIndex_getSegInfo(char *read, unsigned int* sa, BYTE* lcpCompress, 
