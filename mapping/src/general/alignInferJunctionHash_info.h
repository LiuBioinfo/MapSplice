...
public:
	...
	// void categorizeAndOutputJunc_spliceExaminer(string& trueJunc_file, string& falseJunc_file, 
	// 	string& toDetermineJunc_file, SJhash_Info* SJ_ref, Index_Info* indexInfo)
	// {
	...
	// 	int tmpJuncVecSize = alignInferInfoVec.size();
	// 	for(int tmp = 0; tmp < tmpJuncVecSize; tmp++)
	// 	{	
	// 		int tmpJunc_chrNameInt = alignInferInfoVec[tmp].returnChrNameInt();
	// 		int tmpJunc_donerEndPosInt = alignInferInfoVec[tmp].returnDonerEndPos();
	// 		int tmpJunc_acceptorStartPosInt = alignInferInfoVec[tmp].returnAcceptorStartPos();		
	// 		int tmpJunc_supportNum = alignInferInfoVec[tmp].returnAlignInferInfo_supportNum(tmp);
	// 		int tmpJunc_donerAnchorSize = alignInferInfoVec[tmp].returnAnchorSizeMax_doner();
	// 		int tmpJunc_acceptorAnchorSize = alignInferInfoVec[tmp].returnAnchorSizeMax_acceptor();
	// 	}
	// 	trueJunc_ofs.close();
	// 	falseJunc_ofs.close();
	...
	bool searchJuncFromSAMstr(string samStr, Index_Info* indexInfo)
	{
		...
		if(cigarString.find("N") == string::npos)
			return false;		
		...
		int juncNum = tmpSJposPairVec.size();
		for(int tmp = 0; tmp < juncNum; tmp++)
		{
			int tmpSJ_startPos = tmpSJposPairVec[tmp].first;
			int tmpSJ_endPos = tmpSJposPairVec[tmp].second;
			bool foundOrNot = foundInAlignInferJunctionHash(mapChrNameInt, tmpSJ_startPos, tmpSJ_endPos, 0);
			if(foundOrNot)
				return true;
		}
		return false;
	...
	void returnRegionOverlappedSJvec(vector< pair<int,int> >& overlappedSJposPairVec, int tmpJunc_chrNameInt, 
		int tmpJunc_startPos, int tmpJunc_endPos, SJhash_Info* SJ)
	{
		vector< pair<int,int> > tmpCandiSJposPairVec;
		SJ->searchForCandiSJwithinAreas(tmpJunc_chrNameInt, tmpJunc_startPos - 500000, tmpJunc_startPos + 500000,
			tmpJunc_endPos - 500000, tmpJunc_endPos + 500000, tmpCandiSJposPairVec);
		int tmpCandiSJposPairVecSize = tmpCandiSJposPairVec.size();
		for(int tmp = 0; tmp < tmpCandiSJposPairVec.size(); tmp ++)
		{
			int tmpCandiSJ_startPos = tmpCandiSJposPairVec[tmp].first;
			int tmpCandiSJ_endPos = tmpCandiSJposPairVec[tmp].second;
			if(!((tmpCandiSJ_endPos < tmpJunc_startPos)||(tmpCandiSJ_startPos > tmpJunc_endPos)))
				overlappedSJposPairVec.push_back(pair<int,int>(tmpCandiSJ_startPos, tmpCandiSJ_endPos));
		}
	}

	string returnRegionOverlappedSJsStrand(int tmpJunc_chrNameInt, int tmpJunc_startPos, int tmpJunc_endPos, SJhash_Info* SJ)
	{
		vector< pair<int,int> > overlappedSJposPairVec;
		this->returnRegionOverlappedSJvec(overlappedSJposPairVec, tmpJunc_chrNameInt, tmpJunc_startPos, tmpJunc_endPos, SJ);
		int overlappedSJposPairVecSize = overlappedSJposPairVec.size();
		if(overlappedSJposPairVecSize == 0)
			return "N";
		else
		{
			int tmp1stJunc_startPos = overlappedSJposPairVec[0].first;
			int tmp1stJunc_endPos = overlappedSJposPairVec[0].second;
			string tmp1stJuncStrand = this->searchAndReturnAlignInferJuncStrand(tmpJunc_chrNameInt, tmp1stJunc_startPos, tmp1stJunc_endPos);
			if(overlappedSJposPairVecSize == 1)
				return tmp1stJuncStrand;
			else
			{
				for(int tmp = 1; tmp < overlappedSJposPairVecSize; tmp++)
				{
					int tmpCandiJunc_startPos = overlappedSJposPairVec[tmp].first;
					int tmpCandiJunc_endPos = overlappedSJposPairVec[tmp].second;
					string tmpCandiJuncStrand = this->searchAndReturnAlignInferJuncStrand(tmpJunc_chrNameInt, tmpCandiJunc_startPos, tmpCandiJunc_endPos);				
					if(tmpCandiJuncStrand != tmp1stJuncStrand)
						return "N";
				}
				return tmp1stJuncStrand;
			}
	...
	string returnAlignInferInfo_strand(int index)
	{
		return alignInferInfoVec[index].returnStrand();
	}
	...
	string searchAndReturnAlignInferJuncStrand(int tmpChrInt,
		int tmpDonerEndPos, int tmpAcceptorStartPos)
	{
		int tmpIndex = this->searchAndReturnAlignInferInfoVecIndex(
			tmpChrInt, tmpDonerEndPos, tmpAcceptorStartPos);
		if(tmpIndex < 0)
			...
			return this->returnAlignInferInfo_strand(tmpIndex);		
	}
	...
	void insertSJ_chrNamePos_strand_inclusive_fromGeneAnn(int mapChrNameInt, int tmpSJposDonerEnd, 
		int tmpSJposAcceptorStart, string& tmpSJstrand, Index_Info* indexInfo)
	{
		AlignInferJunctionMap::iterator alignInferJuncMapIter 
			= alignInferJunctionMapVec[mapChrNameInt].find(tmpSJposDonerEnd);
		if( alignInferJuncMapIter == (alignInferJunctionMapVec[mapChrNameInt]).end() ) // new donerEndPos to insert
		{
			//cout << "not found" << endl;
			AlignInfer_Info tmpAlignInferInfo;
			tmpAlignInferInfo.initiateAlignInferInfo_chrNamePos_strand(
				mapChrNameInt, tmpSJposDonerEnd, tmpSJposAcceptorStart, tmpSJstrand, indexInfo);
			//cout << "start to insert: " << mapChrNameInt << " " << tmpSJposDonerEnd << " " << tmpSJposAcceptorStart << endl;
			AcceptorStartPos2AlignInferInfoMap tmpAcceptorStartPos2AlignInferInfoMap;
			tmpAcceptorStartPos2AlignInferInfoMap.insert(pair<int, int>(
				tmpSJposAcceptorStart, currentAlignInferInfoVecIndex));
	...
	void insertJuncFromSpliceDataBaseFormatFileVec_chrNamePosOnly(
		vector<string>& spliceDataBaseFormatFileVec, Index_Info* indexInfo)
	{
		for(int tmp = 0; tmp < spliceDataBaseFormatFileVec.size(); tmp++)
			insertJuncFromSpliceDataBaseFormatFile_chrNamePosOnly(
				spliceDataBaseFormatFileVec[tmp], indexInfo);
	}

	void insertJuncFromSpliceDataBaseFormatFile_chrNamePosOnly(
		string& spliceDataBaseFormatFile, Index_Info* indexInfo)
	{
		ifstream junc_ifs(spliceDataBaseFormatFile.c_str());
		while(!junc_ifs.eof())
		{
			string tmpJuncStr;
			getline(junc_ifs, tmpJuncStr);
			...
			if(tmpJuncStr.at(0) == '@')
				continue;
			this->getAlignInferInfoFromSpliceDataBaseFormatStr_InsertIntoAlignInferJunctionHash_chrNamePosOnly(
				tmpJuncStr, indexInfo);					
		}
		junc_ifs.close();
	}	

	void insertJuncFromSpliceDataBaseFormatFile_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr(
		string& spliceDataBaseFormatFile, Index_Info* indexInfo)
	{
		ifstream junc_ifs(spliceDataBaseFormatFile.c_str());
		while(!junc_ifs.eof())
		{
			...
			if(tmpJuncStr.at(0) == '@')
				continue;
			this->getAlignInferInfoFromSpliceDataBaseFormatStr_InsertIntoAlignInferJunctionHash_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr(
				tmpJuncStr, indexInfo);					
		}
		junc_ifs.close();
	}
	...
	void insertJuncFromJuncFile_chrNamePos_strand_inclusive_fromGeneAnn(string& juncFile, Index_Info* indexInfo)
	{
		ifstream junc_ifs(juncFile.c_str());
		while(1)
		{
			...
			if(tmpJuncStr.substr(0,3) != "chr")
				continue;
			//cout << "start to input SAM string: " << tmpJuncStr << endl;
			this->getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePos_strand_inclusive_fromGeneAnn(
				tmpJuncStr, indexInfo);				
		}
		junc_ifs.close();		
	}
	...
	void insertJuncFromJuncFile_chrNamePos_readSupNum_intropolis(string& intropolisFile, Index_Info* indexInfo)
	{
		//cout << "start to do insertJuncFromJuncFile_chrNamePos_readSupNum_intropolis" << endl;
		ifstream junc_ifs(intropolisFile.c_str());
		while(!junc_ifs.eof())
		{
			string tmpJuncStr;
			getline(junc_ifs, tmpJuncStr);
			...
			//	continue;
			//cout << "start to input SAM string: " << tmpJuncStr << endl;
			this->getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePos_readSupNum_intropolis(
				tmpJuncStr, indexInfo);				
		}
		junc_ifs.close();	
	}

	void insertJuncFromJuncFile_chrNamePosOnly_lowSupOnly(string& juncFile, int maxSupNum, Index_Info* indexInfo)
	{
		ifstream junc_ifs(juncFile.c_str());
		while(1)
		{
			...
			if(tmpJuncStr.substr(0,3) != "chr")
				continue;
			//cout << "start to input SAM string: " << tmpJuncStr << endl;
			this->getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePosOnly_lowSupOnly(
				tmpJuncStr, maxSupNum, indexInfo);	
		}
		junc_ifs.close();		
	}
	...
	void insertJuncFromJuncFile_chrNamePos_supportNum_inclusive(string& juncFile, Index_Info* indexInfo)
	// there are only chrName and pos (and junc name, supportNum in some cases) in the file, 
	{
		ifstream junc_ifs(juncFile.c_str());
		while(1)
		{
			...
			if(tmpJuncStr.at(0) == '@')
				continue;
			//cout << "start to input SAM string: " << tmpJuncStr << endl;
			this->getAlignInferInfoFromJunc_InsertIntoAlignInferJunctionHash_chrNamePos_supportNum_inclusive(tmpJuncStr, indexInfo);				
		}
		junc_ifs.close();
	}		
	...
	void insertJuncFromJuncFileVec_chrNamePos_supportNum_inclusive(vector<string>& juncFileVec, Index_Info* indexInfo)
	// inclusive: nolimit on SJ size, it can be any distance, even < 0
	{
		for(int tmp = 0; tmp < juncFileVec.size(); tmp++)
		{	
			string juncFile = juncFileVec[tmp];
	...
	void getAlignInferInfoFromSpliceDataBaseFormatStr_InsertIntoAlignInferJunctionHash_chrNamePosOnly(
	...
	void getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePos_readSupNum_intropolis(
		const string& tmpStr, Index_Info* indexInfo)
	{
		...
		int tmpJunc_readSupNum = atoi(tmpJunc_readSupNumStr.c_str());
		this->insertSJ_chrNamePos_knownSupportNum(tmpJunc_chrNameInt, tmpJunc_startPos, tmpJunc_endPos, tmpJunc_readSupNum, indexInfo);
	}

	void insertSJ_chrNamePos_knownSupportNum(int mapChrNameInt, int tmpSJposDonerEnd, int tmpSJposAcceptorStart, int tmpSJsupportNum, Index_Info* indexInfo)
	{
		AlignInferJunctionMap::iterator alignInferJuncMapIter 
			= alignInferJunctionMapVec[mapChrNameInt].find(tmpSJposDonerEnd);
		if( alignInferJuncMapIter == (alignInferJunctionMapVec[mapChrNameInt]).end() ) // new donerEndPos to insert
		{
			//cout << "not found" << endl;
			AlignInfer_Info tmpAlignInferInfo;
			tmpAlignInferInfo.initiateAlignInferInfo_chrNamePos_supportNum(mapChrNameInt, tmpSJposDonerEnd, tmpSJposAcceptorStart, tmpSJsupportNum, indexInfo);
			//cout << "start to insert: " << mapChrNameInt << " " << tmpSJposDonerEnd << " " << tmpSJposAcceptorStart << endl;
			AcceptorStartPos2AlignInferInfoMap tmpAcceptorStartPos2AlignInferInfoMap;
			tmpAcceptorStartPos2AlignInferInfoMap.insert(pair<int, int>(tmpSJposAcceptorStart, currentAlignInferInfoVecIndex));

			alignInferJunctionMapVec[mapChrNameInt].insert(pair< int, AcceptorStartPos2AlignInferInfoMap> (tmpSJposDonerEnd, tmpAcceptorStartPos2AlignInferInfoMap));
			alignInferInfoVec.push_back(tmpAlignInferInfo);
			currentAlignInferInfoVecIndex ++;
		}
		else // old donerEndPos found, search for acceptorStartPos
		{
			...
			if(acceptorStartPosMapIter == (alignInferJuncMapIter->second).end()) // new acceptor pos found, insert it
			{
				AlignInfer_Info tmpAlignInferInfo;
				tmpAlignInferInfo.initiateAlignInferInfo_chrNamePos_supportNum(mapChrNameInt, tmpSJposDonerEnd, tmpSJposAcceptorStart, tmpSJsupportNum, indexInfo);			
				(alignInferJuncMapIter->second).insert(pair<int, int>(tmpSJposAcceptorStart, currentAlignInferInfoVecIndex));
				alignInferInfoVec.push_back(tmpAlignInferInfo);
				currentAlignInferInfoVecIndex ++;
			}
			else
	...
	void getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePosOnly_lowSupOnly(
		const string& juncStr, int maxSupNum, Index_Info* indexInfo)
	{
		vector<string> juncFieldVec;
		int startLoc = 0;
		...
		if(juncSupNum <= maxSupNum)
			this->insertSJ_chrNamePosOnly(juncChrNameInt, juncDonerEndPos, juncAcceptorStartPos, indexInfo);
	}

	void getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePos_strand_inclusive_fromGeneAnn(
		const string& juncStr, Index_Info* indexInfo)
	{
		vector<string> juncFieldVec;
		int startLoc = 0;
		for(int tmp = 0; tmp < 3; tmp++)
		{
			...
			if(tabLoc == string::npos)
			{
				cout << "error in getAlignInferInfoFromJuncFile_InsertIntoAlignInferJunctionHash_chrNamePos_strand_inclusive_fromGeneAnn" << endl;
				exit(1);
			}
			string tmpJuncField = juncStr.substr(startLoc, tabLoc-startLoc);
			//cout << "tmpJuncField: " << tmpJuncField << endl;
		...
		string juncAcceptorStartPosStr = juncFieldVec[2];
		int juncAcceptorStartPos = atoi(juncAcceptorStartPosStr.c_str());
		string juncStrand = juncFieldVec[3];
		if((juncStrand != "+")&&(juncStrand != "-"))
		{
			cout << "error in juncStrand: " << juncStrand << endl;
			exit(1);
		}
		this->insertSJ_chrNamePos_strand_inclusive_fromGeneAnn(juncChrNameInt, 
			juncDonerEndPos, juncAcceptorStartPos, juncStrand, indexInfo);
	}
	...
	void getAlignInferInfoFromJunc_InsertIntoAlignInferJunctionHash_chrNamePos_supportNum_inclusive(
		const string& juncStr, Index_Info* indexInfo)
	{
		//cout << "getAlignInferInfoFromJunc_InsertIntoAlignInferJunctionHash starts ..." << endl;
		//cout << "juncStr: " << juncStr << endl;
		...
		//int juncAcceptorAnchorSizeMax = 30;
		int tmpJuncSize = juncAcceptorStartPos - juncDonerEndPos - 1;
		this->insertAndInitiateNewSJfromJuncFile_chrNamePos_supportNum(juncChrNameInt, 
			juncDonerEndPos, juncAcceptorStartPos, indexInfo, juncSupportNum);
	}

	void getAlignInferInfoFromSpliceDataBaseFormatStr_InsertIntoAlignInferJunctionHash_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr(
		const string& juncStr, Index_Info* indexInfo)
	{
		//cout << "getAlignInferInfoFromJunc_InsertIntoAlignInferJunctionHash starts ..." << endl;
		//cout << "juncStr: " << juncStr << endl;
		...
		for(int tmp = 0; tmp < 10; tmp++)
		{
			...
			if(tabLoc == string::npos)
			{
				cout << "error in getAlignInferInfoFromSpliceDataBaseFormatStr_InsertIntoAlignInferJunctionHash_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr" << endl;
				exit(1);
			}
			string tmpJuncField = juncStr.substr(startLoc, tabLoc-startLoc);
			//cout << "tmpJuncField: " << tmpJuncField << endl;
		...
		string juncSupportNumStr = juncFieldVec[3];
		int juncSupportNum = atoi(juncSupportNumStr.c_str());
		string juncBranchRatioStr_doner = juncFieldVec[4];
		string juncBranchRatioStr_acceptor = juncFieldVec[5];
		string juncFlankStringStr_ref = juncFieldVec[6];
		string juncFlankStringStr_hap1 = juncFieldVec[7];
		string juncFlankStringStr_hap2 = juncFieldVec[8];
		string juncAnchorSizeStr_doner = juncFieldVec[9];
		string juncAnchorSizeStr_acceptor = juncFieldVec[10];

		double juncBranchRatio_doner = atof(juncBranchRatioStr_doner.c_str());
		double juncBranchRatio_acceptor = atof(juncBranchRatioStr_acceptor.c_str());
		int juncAnchorSize_doner = atoi(juncAnchorSizeStr_doner.c_str());
		int juncAnchorSize_acceptor = atoi(juncAnchorSizeStr_acceptor.c_str());

		this->insertAndInitiateNewSJfromJuncFile_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr(juncChrNameInt, 
			juncDonerEndPos, juncAcceptorStartPos, indexInfo, juncSupportNum, juncAnchorSize_doner, juncAnchorSize_acceptor,
			juncBranchRatio_doner, juncBranchRatio_acceptor, juncFlankStringStr_ref, juncFlankStringStr_hap1, juncFlankStringStr_hap2);
	}
	...
	void insertAndInitiateNewSJfromJuncFile_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr(
		int mapChrNameInt, int tmpSJposDonerEnd, int tmpSJposAcceptorStart, Index_Info* indexInfo, int tmpJuncSupportNum, 
		int tmpJuncDonerAnchorSizeMax, int tmpJuncAcceptorAnchorSizeMax, double tmpJuncDonerBranchRatio, double tmpJuncAcceptorBranchRatio,
		string& tmpJuncFlkStr_ref, string& tmpJuncFlkStr_hap1, string& tmpJuncFlkStr_hap2)
	{
		AlignInfer_Info tmpAlignInferInfo;
		tmpAlignInferInfo.initiateAlignInferInfo_chrNamePos_supportNum_anchorSize_branchRatio_hapFlkStr(
			mapChrNameInt, tmpSJposDonerEnd, tmpSJposAcceptorStart, tmpJuncSupportNum, 
			tmpJuncDonerAnchorSizeMax, tmpJuncAcceptorAnchorSizeMax, indexInfo, 
			tmpJuncDonerBranchRatio, tmpJuncAcceptorBranchRatio, tmpJuncFlkStr_ref, tmpJuncFlkStr_hap1, tmpJuncFlkStr_hap2);
