...
public:
		...
		/*
		bool detectBestSpliceSite_includeSNP(//_prefer_canonical_lessMismatch
			int toFixSeqLocInRead_start, int toFixSeqLocInRead_end,
			int toFixSeqMapPos_doner, int toFixSeqMapPos_acceptor, 
			const string& readSeq_inProcess, 
			Index_Info* indexInfo, int chrNameInt, 
			int max_allowed_mismatchNum,
			bool annotation_provided_bool, 
			bool Do_annotation_only_bool, Annotation_Info* annotationInfo, SNPhash_Info& snpHashInfo)	
		{
			vector<int> tmpSNPposVec;
			bool SNPwithinRegionOrNot_bool = snpHashInfo.searchSNPvecWithinRegion(int chrNameInt, 
				toFixSeqMapPos_doner, toFixSeqMapPos_acceptor, vector<int>& tmpSNPposVec);
			if(!SNPwithinRegionOrNot_bool)
			{
				bool tmp_detectBestSpliceSite_asNotSNP_bool = this->detectBestSpliceSite(
					toFixSeqLocInRead_start, toFixSeqLocInRead_end, toFixSeqMapPos_doner, 
					toFixSeqMapPos_acceptor, readSeq_inProcess, indexInfo, chrNameInt, 
					max_allowed_mismatchNum, annotation_provided_bool, 
					Do_annotation_only_bool, annotationInfo);
				return tmp_detectBestSpliceSite_asNotSNP_bool;
			}
			else
			{
				this->scanGenomeAndReadSeq_includeSNP(
					toFixSeqLocInRead_start, toFixSeqLocInRead_end,
					toFixSeqMapPos_doner, toFixSeqMapPos_acceptor, 
					readSeq_inProcess, indexInfo, chrNameInt, 
					max_allowed_mismatchNum, snpHashInfo);	
				this->generateSpliceSiteVec_includeSNP(
					toFixSeqLocInRead_start, toFixSeqLocInRead_end,
					toFixSeqMapPos_doner, toFixSeqMapPos_acceptor,
					indexInfo, chrNameInt, max_allowed_mismatchNum,
					annotation_provided_bool, Do_annotation_only_bool, 
					annotationInfo, snpHashInfo);
				bestSpliceSiteIndex = this->selectBestSpliceSite();
				int maxMismatchNum_semi_noncanonical = max_allowed_mismatchNum;
