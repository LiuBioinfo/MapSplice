...
void parseMergedFa(BuildIndexParameter_info* buildIndexInfo, string mergedFaPath)
{
	ifstream mergedFa_ifs(mergedFaPath.c_str());
	string tmpChrSeq;
	int refSeqNum = 0;
	while(!mergedFa_ifs.eof())
	{
		string tmpStr;
		getline(mergedFa_ifs, tmpStr);
		if(tmpStr == "")
			break;
		if(tmpStr.at(0) == '>')
		{	
			refSeqNum++;
			tmpStr += ".fa";
			(buildIndexInfo->chrNameStrVec).push_back(tmpStr.substr(1));
			if(refSeqNum > 1)
			{
				int tmpChrSeqLength = tmpChrSeq.length();
				(buildIndexInfo->chromLengthVec).push_back(tmpChrSeqLength);
				(buildIndexInfo->chromSeqVec).push_back(tmpChrSeq);		
			}
			tmpChrSeq = "";
		}
		else
	...
	(buildIndexInfo->chromSeqVec).push_back(tmpChrSeq);
	mergedFa_ifs.close();
	cout << "total refSeqNum: " << refSeqNum << endl;
}
...
int main(int argc, char** argv)
{
	...
	// }
	// mergedFa_ifs.close();
	parseMergedFa(buildIndexInfo, mergedFaFile);
